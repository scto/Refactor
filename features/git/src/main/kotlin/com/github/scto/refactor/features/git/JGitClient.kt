...
class JGitClient : GitClient {
    override suspend fun clone(
        ...
        Timber.tag("GitClient")
            .d(
                "Cloning $url into ${directory.absolutePath} | Branch: ${branch ?: "default"} | Submodules: $cloneSubmodules"
            )
        try {
            val cloneCommand =
                Git.cloneRepository()
        ...
        try {
            Git.open(directory).use { git ->
                ...
                if (result.isSuccessful) {
                    ...
                    Timber.tag("GitClient")
                        .w("Pull not successful. Merge status: ${result.mergeResult.mergeStatus}")
                }
            }
        } catch (e: Exception) {
    ...
    /** Pushes committed changes to the remote repository. Requires authentication. */
