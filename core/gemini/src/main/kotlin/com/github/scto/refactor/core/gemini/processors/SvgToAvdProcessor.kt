...
object SvgToAvdProcessor : Processor {
    ...
    override val description: String =
        "Konvertiert .svg-Grafiken in das Android-spezifische XML-Format."
    ...
    override fun process(config: RefactoringConfig): Flow<String> = flow {
        ...
        if (drawableDir == null) {
            emit(
                "FEHLER: Konnte kein 'drawable'-Verzeichnis im Projekt finden, um die Ergebnisse zu speichern."
            )
            return@flow
        }
                ...
                result.fold(
                    onSuccess = { avdContent ->
                        if (avdContent.contains("<vector")) {
                            val newFileName =
                                svgFile.nameWithoutExtension.lowercase().replace("-", "_") + ".xml"
                            val newFile = File(drawableDir, newFileName)
                            newFile.writeText(avdContent)
                            emit("     Erfolgreich nach ${newFile.name} konvertiert.")
                        } else {
                            emit(
                                "     FEHLER bei der Konvertierung von ${svgFile.name}: Die Antwort war kein gültiges Vector Drawable."
                            )
                        }
                    },
                    onFailure = { error ->
                        emit(
                            "     KRITISCHER FEHLER bei der Verarbeitung von ${svgFile.name}: ${error.message}"
                        )
                    },
                )
            }
