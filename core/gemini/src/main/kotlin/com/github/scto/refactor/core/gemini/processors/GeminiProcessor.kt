...
object GeminiProcessor {
   private val apiKey = BuildConfig.GEMINI_API_KEY.ifBlank {
   ...
   private suspend fun ask(prompt: String): Result<String> {
       return try {
           ...
           val cleanedText = text.trim().replace(Regex("```[a-zA-Z]*\n?"), "").replace("```", "").trim()
   ...
   suspend fun refactorGradleToKts(groovyCode: String): Result<String> {
       val prompt = "Du bist ein Gradle-Experte. Konvertiere das folgende Groovy-basierte Gradle-Skript in ein Kotlin DSL-Skript (.kts). Behalte alle Konfigurationen exakt bei. Gib NUR das reine Kotlin DSL zurück.\n\nGroovy Gradle Code:\n```groovy\n$groovyCode\n```"
       return ask(prompt)
   }

   suspend fun convertJavaToKotlin(javaCode: String): Result<String> {
       val prompt = "Du bist ein Experte für Kotlin. Konvertiere den folgenden Java-Code nach idiomatischem, modernem Kotlin. Gib NUR den reinen Kotlin-Code zurück.\n\nJava Code:\n```java\n$javaCode\n```"
       return ask(prompt)
   }

   suspend fun analyzeKotlinStyle(fileName: String, code: String): Result<String> {
       val prompt = "Analysiere den folgenden Kotlin-Code aus der Datei `$fileName`. Refaktorisiere ihn, um ihn idiomatischer, null-sicherer und lesbarer zu machen. Gib nur den refaktorisierten Code zurück.\n\nOriginal Code:\n```kotlin\n$code\n```"
       return ask(prompt)
   }

   suspend fun analyzeAndRefactorManifest(manifestXml: String): Result<String> {
       val prompt = "Analysiere die folgende `AndroidManifest.xml`-Datei. Korrigiere `android:exported`-Flags für Android 12+ und stelle sicher, dass die Datei wohlgeformt ist. Gib die korrigierte `AndroidManifest.xml` zurück. Gib NUR den XML-Code zurück.\n\nAndroidManifest.xml:\n```xml\n$manifestXml\n```"
       return ask(prompt)
   }

   suspend fun selfModernizer(files: List<Pair<String, String>>): Result<String> {
       val fileContentString = files.joinToString("\n\n---\n\n") { (name, content) -> "// Datei: $name\n$content" }
       val prompt = "Du bist ein Weltklasse-Softwarearchitekt. Analysiere die folgenden Dateien eines Android-Tools namens \"Refactor\". Deine Aufgabe ist eine Meta-Analyse: 1. Analysiere die Gesamtarchitektur. 2. Schlage die NÄCHSTE, wichtigste Refactoring-Funktion vor. 3. Finde Code-Smells. Gib eine kurze Zusammenfassung deiner drei wichtigsten Erkenntnisse zurück.\n\nProjekdateien:\n```\n$fileContentString\n```"
       return ask(prompt)
   }

   suspend fun convertSvgToAvd(svgContent: String): Result<String> {
       val prompt = """Du bist ein Experte für Android-Grafikformate. Konvertiere den folgenden SVG-Code in ein Android Vector Drawable (AVD) XML. Beachte dabei die folgenden Konvertierungsregeln:
   ...
   suspend fun convertThemeToMaterial3(oldThemesXml: String, oldColorsXml: String): Result<String> {
       val prompt = "Du bist ein Android UI/UX-Experte. Konvertiere das folgende alte AppCompat-Theme und die dazugehörigen Farben in ein modernes Material 3 Theme. Parent-Theme soll `Theme.Material3.DayNight.NoActionBar` sein. Gib das Ergebnis als eine einzige, vollständige XML-Datei zurück.\n\nAlte `themes.xml`:\n```xml\n$oldThemesXml\n```\n\nAlte `colors.xml`:\n```xml\n$oldColorsXml\n```"
       return ask(prompt)
   }
