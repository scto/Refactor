...
object PackageNameProcessor : Processor {
    ...
    override val description: String = "Benennt Paketnamen in allen relevanten Dateien und Verzeichnissen um."
    ...
    override fun process(config: RefactoringConfig): Flow<String> = flow {
        if (config.oldPackageName.isBlank() || config.newPackageName.isBlank()) {
			...
			Timber.tag("PackageNameProcessor").d("FEHLER: Alter und neuer Paketname dürfen nicht leer sein. Überspringe.")
            return@flow
        }

        emit("Starte Umbenennung von '${config.oldPackageName}' zu '${config.newPackageName}'...")
		Timber.tag("PackageNameProcessor").d("Starte Umbenennung von '${config.oldPackageName}' zu '${config.newPackageName}'...")
        ...
        val sourceDirs = config.projectRoot.walkBottomUp().filter { it.isDirectory && it.path.contains(oldPath) }
        sourceDirs.forEach { dir ->
            val newDir = File(dir.path.replace(oldPath, newPath))
            if (dir.renameTo(newDir)) {
                emit("  -> Verzeichnis umbenannt: ${dir.name} -> ${newDir.name}")
				Timber.tag("PackageNameProcessor").d("Verzeichnis umbenannt: ${dir.name} -> ${newDir.name}")
            }
        }
    }
}