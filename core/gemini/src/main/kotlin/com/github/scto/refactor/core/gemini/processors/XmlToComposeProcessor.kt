...
object XmlToComposeProcessor : Processor {
    ...
    override fun process(config: RefactoringConfig): Flow<String> = flow {
        ...
        val composeDir =
            config.projectRoot
                .resolve("app/src/main/java/com/github/scto/refactor/ui/converted")
                .apply { mkdirs() }
        ...
        for (file in xmlFiles) {
            ...
            result.fold(
                onSuccess = { composeCode ->
                    val newFileName =
                        file.nameWithoutExtension.replaceFirstChar { it.titlecase() } + "Screen.kt"
                    val newFile = File(composeDir, newFileName)
                    newFile.writeText(
                        "package com.github.scto.refactor.ui.converted\n\n$composeCode"
                    )
