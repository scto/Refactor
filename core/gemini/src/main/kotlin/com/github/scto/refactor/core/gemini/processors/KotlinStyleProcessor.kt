...
object KotlinStyleProcessor : Processor {
    ...
    override val description: String = "Analysiert Kotlin-Dateien und schlägt idiomatische Verbesserungen vor."

    override fun process(config: RefactoringConfig): Flow<String> = flow {
        val kotlinFiles = config.projectRoot.walk().filter { it.isFile && it.extension == "kt" }.toList()
        emit("${kotlinFiles.size} Kotlin-Dateien werden analysiert...")
        Timber.tag("KotlinStyleProcessor").d("${kotlinFiles.size} Kotlin-Dateien werden analysiert...")
                ...
                result.fold(
                    onSuccess = { suggestion ->
                        if (suggestion.isNotBlank() && suggestion != fileContent) {
                            ...
                            Timber.tag("KotlinStyleProcessor").d("VORSCHLAG für ${file.name}:\n$suggestion\n--------------------")
                        }
                    },
                    onFailure = {
