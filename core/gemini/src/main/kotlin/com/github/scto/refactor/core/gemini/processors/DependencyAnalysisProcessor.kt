...
class DependencyAnalysisProcessor @Inject constructor() : Processor {
    ...
    override val description: String = "Sucht nach veralteten Abhängigkeiten und schlägt neue Versionen vor."

    /**
     * KORREKTUR: Die Funktion gibt jetzt einen Flow<String> zurück, um der
     * Processor-Schnittstelle zu entsprechen. Status-Updates und Ergebnisse
     * werden mit 'emit' gesendet.
     */
    override fun process(config: RefactoringConfig): Flow<String> = flow {
        ...
        dependencies.forEach { (group, name, version) ->
            ...
            if (latestVersion != null && latestVersion != version) {
                emit("Empfehlung: Aktualisieren Sie '$group:$name' von Version '$version' auf '$latestVersion'.")
    ...
    private suspend fun fetchLatestVersion(group: String, name: String): String? {
        ...
        return try {
            val url = "https://search.maven.org/solrsearch/select?q=g:\"$group\"+AND+a:\"$name\"&rows=1&wt=json"
