...
class RefactorViewModel @Inject constructor(private val orchestrator: RefactoringOrchestrator) :
    ...
    fun onEvent(event: UiEvent) {
        when (event) {
            is UiEvent.ProjectPathChanged ->
                _uiState.update { it.copy(projectPath = event.newPath) }
            is UiEvent.TargetPackageNameChanged ->
                _uiState.update { it.copy(targetPackageName = event.newName) }
            is UiEvent.RefactoringOptionToggled ->
                toggleRefactoringOption(event.option, event.isEnabled)
        ...
        refactoringJob =
            viewModelScope.launch {
                try {
                    ...
                    _uiState.update {
                        it.copy(currentStep = "Refactoring erfolgreich abgeschlossen!")
                    }
                } catch (e: Exception) {
                    _uiState.update {
                        it.copy(
                            currentStep = "Fehler: ${e.localizedMessage ?: "Unbekannter Fehler"}"
                        )
                    }
                } finally {
    ...
    private fun applySuggestion(suggestion: StyleSuggestion) {
        viewModelScope.launch {
            try {
                ...
                _uiState.update { currentState ->
                    currentState.copy(styleSuggestions = currentState.styleSuggestions - suggestion)
                }
            } catch (e: Exception) {
                _uiState.update {
