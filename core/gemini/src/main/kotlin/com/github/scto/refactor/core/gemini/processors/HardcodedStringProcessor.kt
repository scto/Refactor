...
object HardcodedStringProcessor : Processor {
   ...
   override val description: String = "Findet hartcodierte Texte, die in `strings.xml` extrahiert werden sollten."
   ...
   private val commonFalsePositives = listOf("import", "@", "Timber", "Log.", "http://", "https://", "TAG", "=")
               ...
               "xml" -> {
                   file.readLines().forEach { line ->
                       xmlStringRegex.findAll(line).forEach { match ->
                           ...
                           emit("  -> VORSCHLAG in ${file.name}: Hartcodierter Text gefunden: \"$hardcodedString\".")
                           emit("     Extrahiere dies in `strings.xml`: <string name=\"example_name\">$hardcodedString</string>")
                       }
                   }
               }
               "kt" -> {
                   file.readLines().forEachIndexed { index, line ->
                       ...
                       if (commonFalsePositives.none { trimmedLine.contains(it) }) {
                           kotlinStringRegex.findAll(line).forEach { match ->
                               issuesFound++
                               val hardcodedString = match.groupValues[1]
                               emit("  -> VORSCHLAG in ${file.name} (Zeile ${index + 1}): Potenziell hartcodierter String: \"$hardcodedString\".")
                               emit("     Prüfen, ob `context.getString(R.string.example_name)` verwendet werden sollte.")
                           }
                       }
                   }
               }
